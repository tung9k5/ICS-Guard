...
const CustomerIncidents = () => {
  ...
  const statusLabel = { open: t('customer.status.open'), investigating: t('customer.status.investigating'), resolved: t('customer.status.resolved'), closed: t('customer.status.closed') };
      ...
      <div style={{ display: 'flex', gap: '1.4286rem' }}>
        ...
        <div style={{ flex: selected ? '0 0 55%' : '1' }}>
          <div style={{ background: 'var(--white)', borderRadius: '0.8571rem', border: '0.0714rem solid var(--slate-200)', overflow: 'hidden', boxShadow: '0 0.2857rem 0.4286rem -0.0714rem rgba(0, 0, 0, 0.05)' }}>
            {loading ? (
              <div style={{ padding: '2.8571rem', textAlign: 'center', color: 'var(--slate-500)' }}>              {t('customer.common.loading')}</div>
            ) : incidents.length === 0 ? (
        ...
        {selected && (
          <div style={{ flex: '0 0 42%', background: 'var(--white)', borderRadius: '0.8571rem', border: '0.0714rem solid var(--slate-200)', padding: '1.7143rem', alignSelf: 'flex-start', boxShadow: '0 0.2857rem 0.4286rem -0.0714rem rgba(0, 0, 0, 0.05)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.1429rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.1429rem', fontWeight: '600', color: 'var(--slate-900)' }}>{t('customer.incidents.detail_title')}</h3>
            ...
            <div style={{ marginBottom: '1.1429rem' }}>
              <p style={{ fontSize: '0.9286rem', color: 'var(--slate-500)', margin: '0 0 0.2857rem' }}>{t('customer.incidents.lbl_title')}</p>
              <p style={{ fontSize: '1.0714rem', fontWeight: '600', color: 'var(--slate-900)', margin: 0 }}>{selected.title}</p>
            </div>
            {selected.description && (
              <div style={{ marginBottom: '1.1429rem' }}>
                <p style={{ fontSize: '0.9286rem', color: 'var(--slate-500)', margin: '0 0 0.2857rem' }}>{t('customer.incidents.lbl_description')}</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.1429rem', background: 'var(--slate-50)', padding: '1.1429rem', borderRadius: '0.5714rem' }}>
              <div>
                <p style={{ fontSize: '0.9286rem', color: 'var(--slate-500)', margin: '0 0 0.4286rem' }}>{t('customer.incidents.lbl_status')}</p>
              ...
              <div>
                <p style={{ fontSize: '0.9286rem', color: 'var(--slate-500)', margin: '0 0 0.4286rem' }}>{t('customer.incidents.lbl_created_at')}</p>
