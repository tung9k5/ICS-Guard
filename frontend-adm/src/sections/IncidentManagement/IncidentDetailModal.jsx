        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div>
            <h4 style={{ margin: 0, color: 'var(--slate-900)', fontSize: '1.05rem', fontWeight: 600 }}>
              {t('incidents.ai.result_title', 'Kết quả Phân tích AI')}
            </h4>
        ...
        {aiResult.risk_level && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', backgroundColor: 'white', padding: '0.375rem 0.75rem', borderRadius: '2rem', border: '1px solid var(--slate-300)', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
            <span style={{ fontSize: '0.75rem', color: 'var(--slate-700)', fontWeight: 600, textTransform: 'uppercase' }}>
              {t('incidents.ai.risk_level', 'Mức rủi ro')}:
            </span>
            <span style={{ 
              fontWeight: 700, 
      ...
      <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div>
           <h5 style={{ fontSize: '0.85rem', color: 'var(--slate-700)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 0.5rem 0', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
             <Info size={14} /> {t('incidents.ai.log_summary', 'Tóm tắt sự kiện')}
           </h5>
        ...
        {aiResult.attack_reasoning && (
          <div>
            <h5 style={{ fontSize: '0.85rem', color: 'var(--slate-700)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 0.5rem 0', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
              <Target size={14} /> {t('incidents.ai.attack_reasoning', 'Phân tích nguyên nhân & Đánh giá')}
            </h5>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem' }}>
           ...
           {aiResult.remediation_advice?.length > 0 && (
             <div>
               <h5 style={{ fontSize: '0.85rem', color: 'var(--slate-700)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 0.75rem 0', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                 <ShieldAlert size={14} /> {t('incidents.ai.remediation_advice', 'Khuyến nghị xử lý')}
               </h5>
           ...
           {aiResult.mitre_attack_mappings?.length > 0 && (
             <div>
               <h5 style={{ fontSize: '0.85rem', color: 'var(--slate-700)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 0.75rem 0', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                 <Tag size={14} /> {t('incidents.ai.mitre_attack', 'Phân loại MITRE ATT&CK')}
               </h5>
...
const IncidentDetailModal = ({ incident: incidentData, onClose, onAiAnalyze, onRefresh }) => {
  ...
  useEffect(() => {
    ...
    pollingInterval.current = setInterval(async () => {
      try {
        ...
        if (currentStatus === AI_STATUS.COMPLETED) {
          clearInterval(pollingInterval.current);
          setAiStatus(AI_STATUS.COMPLETED);
          toast.success(t('incidents.ai.analyze_success_msg', { 
            title: incident.title || incident.incident_code,
            defaultValue: `Phân tích thành công cảnh báo ${incident.title || incident.incident_code}` 
          }));
          if (onRefresh) onRefresh(incident._id || incident.id);
        } else if (currentStatus === AI_STATUS.FAILED) {
          clearInterval(pollingInterval.current);
          setAiStatus(AI_STATUS.FAILED);
          toast.error(t('incidents.ai.analyze_error_msg', {
            title: incident.title || incident.incident_code,
            defaultValue: `Phân tích cảnh báo ${incident.title || incident.incident_code} thất bại.`
          }));
          if (onRefresh) onRefresh(incident._id || incident.id);
        }
      } catch (error) {
  ...
  }, [aiStatus, incident?._id, onRefresh, t]);
  ...
  const handleAiClick = () => {
    ...
    if (onAiAnalyze) {
      onAiAnalyze(incident._id || incident.id);
    }
  };
    ...
    <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', width: '100%' }}>
      {onAiAnalyze && (
        <VButton
          ...
          disabled={aiStatus === AI_STATUS.PROCESSING}
        >
          {aiStatus === AI_STATUS.PROCESSING ? (
            <><Loader2 size={16} className="animate-spin" style={{ marginRight: '0.375rem' }} />{t('incidents.ai.btn_analyzing', 'Đang phân tích...')}</>
          ) : (
            <><Bot size={16} style={{ marginRight: '0.375rem' }} />{t('incidents.ai.btn_analyze', 'Phân tích AI')}</>
          )}
        </VButton>
      )}
      <VButton variant="outline" onClick={onClose}>
        {t('incidents.detail.close_btn', 'Đóng')}
      </VButton>
    </div>
  );
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        {deviceAlertHistory.length > 0 && (
          <div style={{ backgroundColor: 'var(--slate-50)', padding: '1.25rem', borderRadius: '0.5rem', border: '1px solid var(--slate-200)' }}>
            <h4 style={{ margin: '0 0 1rem 0', color: 'var(--slate-800)', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1rem' }}>
              ...
              {t('incidents.detail.device_alert_history', 'Lịch sử Cảnh báo Thiết bị')}
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        ...
        {aiStatus === AI_STATUS.COMPLETED && liveAiResult ? (
          ...
          <div style={{ backgroundColor: 'var(--slate-50)', padding: '1.25rem', borderRadius: '0.5rem', border: '1px solid #bee3f8', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div>
              <div style={{ fontWeight: 600, color: 'var(--slate-800)' }}>{t('incidents.ai.status_processing', 'Đang phân tích AI...')}</div>
              <div style={{ fontSize: '0.85rem', color: 'var(--slate-500)' }}>{t('incidents.ai.analyzing_msg', 'Kết quả sẽ hiển thị ngay khi AI hoàn thành.')}</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: 'var(--red-50)', padding: '1.25rem', borderRadius: '0.5rem', border: '1px solid var(--red-200)' }}>
            <div style={{ fontWeight: 600, color: 'var(--red-700)' }}>❌ {t('incidents.ai.status_failed', 'Lỗi phân tích AI')}</div>
            <div style={{ fontSize: '0.85rem', color: 'var(--red-500)', marginTop: '0.25rem' }}>{t('incidents.ai.failed_msg', 'Phân tích AI thất bại. Vui lòng thử lại sau.')}</div>
          </div>
        ) : null}
        ...
        {timeline.length > 0 && (
          <div style={{ backgroundColor: 'var(--slate-50)', padding: '1.25rem', borderRadius: '0.5rem', border: '1px solid var(--slate-200)' }}>
            <h4 style={{ margin: '0 0 1rem 0', color: 'var(--slate-800)', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1rem' }}>
              <Target size={18} className="text-primary" />
              {t('incidents.detail.timeline', 'Tiến trình xử lý')}
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {history.length > 1 && (
          <div style={{ backgroundColor: 'var(--slate-50)', padding: '1.25rem', borderRadius: '0.5rem', border: '1px solid var(--slate-200)' }}>
            <h4 style={{ margin: '0 0 1rem 0', color: 'var(--slate-800)', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1rem' }}>
              <Activity size={18} className="text-primary" />
              {t('incidents.detail.simulation_history', 'Lịch sử mô phỏng')}
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {history.map((h, index) => {
                  ...
                  <div key={h._id || index} style={{ padding: '0.875rem', backgroundColor: 'white', border: '1px solid var(--slate-200)', borderRadius: '0.5rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                      <span style={{ fontWeight: 600, color: 'var(--slate-700)' }}>
                        {t('incidents.detail.simulation_history', 'Mô phỏng')} #{history.length - index}
                      </span>
