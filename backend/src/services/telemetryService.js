...
class TelemetryService {
  ...
  async controlAttack(deviceId, attackType) {
    ...
    if (attackType === ATTACK_TYPES.STOP && device.status === DEVICE_STATUSES.ISOLATED) {
      device.status = DEVICE_STATUSES.ACTIVE;
      await deviceRepository.updateById(deviceId, { status: DEVICE_STATUSES.ACTIVE });
    } else if (attackType !== ATTACK_TYPES.STOP) {
      device.status = DEVICE_STATUSES.ISOLATED;
      await deviceRepository.updateById(deviceId, { status: DEVICE_STATUSES.ISOLATED });
    }
