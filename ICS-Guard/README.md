...
*   `[frontend](file:///d:/New%20folder/ICS-Guard/frontend)`: Giao diện giám sát SOC (ReactJS + Vite).
*   `[backend](file:///d:/New%20folder/ICS-Guard/backend)`: API dịch vụ và phân tích sự cố (NodeJS Express).
*   `[iot](file:///d:/New%20folder/ICS-Guard/iot)`: Chứa các thành phần mô phỏng thiết bị IoT/ICS bao gồm:
    *   `[iot/simulator](file:///d:/New%20folder/ICS-Guard/iot/simulator)`: Bộ giả lập thiết bị công nghiệp và kịch bản tấn công.
    *   `[iot/agents](file:///d:/New%20folder/ICS-Guard/iot/agents)`: Bộ giả lập thực thể (PLC, Sensor, Smart Meter).
    *   `[iot/payloads](file:///d:/New%20folder/ICS-Guard/iot/payloads)`: Sinh dữ liệu mẫu.
    *   `[iot/certificates](file:///d:/New%20folder/ICS-Guard/iot/certificates)`: Chứng chỉ bảo mật phục vụ kênh truyền TLS 1.3.
*   `[infrastructure](file:///d:/New%20folder/ICS-Guard/infrastructure)`: Tệp cấu hình các container (Nginx, Mosquitto TLS, MongoDB init).
*   `[docs](file:///d:/New%20folder/ICS-Guard/docs)`: Chứa toàn bộ báo cáo, hướng dẫn công việc và kế hoạch KPI.
