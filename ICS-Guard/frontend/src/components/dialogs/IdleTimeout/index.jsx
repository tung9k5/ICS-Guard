...
const IdleTimeout = () => {
  ...
  return (
    <VDialog
      ...
      closable={false}
    >
      <div className="idle-timeout-body">
        <div className="idle-icon-badge">
          <Clock size={36} />
        </div>
        <h3 className="idle-title">
          {t('common.idle_timeout.title', 'Bạn còn ở đó không?')}
        </h3>
        <p className="idle-description">
          {t('common.idle_timeout.description_1', 'Phiên đăng nhập sẽ tự động đăng xuất sau ')}
          <span className="idle-countdown-box">
            {countdown} giây
          </span>
          {t('common.idle_timeout.description_2', ' nữa do không có hoạt động.')}
        </p>
        <VButton 
          variant="primary" 
