...
import { CheckCircle, XCircle, Trash2, ShieldAlert, ChevronDown, ChevronUp, Bot } from 'lucide-react';
import ActionMenu from '@/components/ActionMenu';
import VCheckbox from '@/components/VCheckbox';
import VNoData from '@/components/VNoData';
import { ALERT_SEVERITIES, ALERT_STATUSES } from '@/constants/alertConstants';

const AlertList = ({ alerts, onUpdateStatus, onDelete, onAiTriage, selectedIds, onSelect, onSelectAll }) => {
  ...
  const getActions = (alert) => [
    { icon: Bot, label: 'Trợ lý AI Triage', onClick: () => onAiTriage && onAiTriage(alert) },
              ...
              return (
                <tr key={alert._id} className={isSelected ? 'selected-row' : ''}>
                  ...
                  <td>
                    ...
                    {(alert.status === 'false_positive' || alert.severity === 'LOW' || alert.severity === 'MEDIUM') && (
                      <span
                        title="AI Phân tích: Có khả năng là Cảnh báo giả (False Positive) dựa trên tần suất và hành vi hệ thống"
                        style={{
                          display: 'inline-flex',
                          alignItems: 'center',
                          gap: '3px',
