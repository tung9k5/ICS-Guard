...
const UserForm = ({ user, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.email || !formData.role || (isEdit && !formData.username)) {
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (response && response.data && response.data.tempPassword) {
          setNewUserCredentials({
            username: response.data.username,
            tempPassword: response.data.tempPassword,
            role: formData.role
          });
          // onSuccess will be called when modal closes
        } else {
