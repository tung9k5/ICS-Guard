...
const HardwareSimulator = () => {
  ...
  const fetchDevices = useCallback(async (isInit = false) => {
    try {
      ...
      // Filter out offline/decommissioned devices (they are physically unplugged)
      const connectedDevices = list.filter(d => d.status !== 'offline' && d.status !== 'decommissioned');
    ...
    return (
      <div key={node._id} className="tree-node" style={{ '--depth-left': `${14 + depth * 14}px` }}>
        ...
        >
          ...
          <div className={`tree-node-status-dot ${node.status || 'active'}`} />
  ...
  const renderCanvasNode = (node) => {
    ...
    return (
      <div
        key={node._id}
        className={`canvas-node status-${status}${isSelected ? ' selected' : ''}`}
        ...
        <div className="canvas-node-body">
          ...
          >
            ...
            <span
              ...
              style={{ background: status === 'active' ? '#10b981' : status === 'isolated' ? '#f59e0b' : '#ef4444' }}
            />
          </div>
                      ...
                      {cables.map(c => {
                        ...
                        return (
                          <line
                            ...
                            stroke={isQ ? '#ef4444' : isI ? '#f59e0b' : '#334155'}
                            strokeWidth={isQ ? 2.5 : 1.5}
                            strokeDasharray={isI ? '5,4' : 'none'}
            ...
            <div className="config-scroll">
              ...
              <div className="config-field-group">
                ...
                <div className="config-field">
                  ...
                  <span className={`status-badge ${selectedDevice.status || 'active'}`}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'currentColor', display: 'inline-block' }} />
                    {selectedDevice.status === 'active' ? 'Connected' : selectedDevice.status === 'isolated' ? 'Isolated' : 'Alert'}
                  </span>
                </div>
