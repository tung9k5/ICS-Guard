...
import { disconnectSocket, refreshSocketAuthentication } from '@/services/socket';
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      if (originalRequest.url.includes('/auth/refresh')) {
        ...
        if (accessTokenKey === 'access_token') disconnectSocket();
      ...
      try {
        ...
        if (data && data.access_token) {
          ...
          if (accessTokenKey === 'access_token') refreshSocketAuthentication();
