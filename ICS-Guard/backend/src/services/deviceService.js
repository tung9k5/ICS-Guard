...
class DeviceService {
  ...
  async isolate(id, actor, ipAddress) {
    const device = await deviceRepository.findById(id);
    if (!device) throw new AppError('Device not found', 404);
    if (device.status === 'isolated') throw new AppError('Device is already isolated', 400);
  ...
  async unisolate(id, actor, ipAddress) {
    const device = await deviceRepository.findById(id);
    if (!device) throw new AppError('Device not found', 404);
    if (device.status === 'active' || device.status === 'online') throw new AppError('Device is already active', 400);

    const updatedDevice = await deviceRepository.updateById(id, { status: 'active' });
