...
export const getIncidentStatusLabel = (status, t) => {
  ...
  switch (status) {
    case INCIDENT_STATUS.OPEN: 
      return translationFn('customer.incidents.list.status_open', 'Mở');
    case INCIDENT_STATUS.INVESTIGATING: 
      return translationFn('customer.incidents.list.status_investigating', 'Đang điều tra');
    case INCIDENT_STATUS.INVESTIGATED: 
      return translationFn('customer.incidents.list.status_investigated', 'Đã điều tra');
    case INCIDENT_STATUS.REMEDIATED: 
      return translationFn('customer.incidents.list.status_remediated', 'Đã xử lý (Remediated)');
    case INCIDENT_STATUS.CLOSED: 
      return translationFn('customer.incidents.list.status_closed', 'Đóng');
    default: 
      return status;
  }
};
...
export const getAiStatusLabel = (aiStatus, t) => {
  ...
  switch (aiStatus) {
    case AI_STATUS.PROCESSING:
      return translationFn('customer.incidents.ai.status_processing', 'Đang phân tích...');
    case AI_STATUS.COMPLETED:
      return translationFn('customer.incidents.ai.status_completed', 'Đã phân tích');
    case AI_STATUS.FAILED:
      return translationFn('customer.incidents.ai.status_failed', 'Lỗi phân tích');
    case AI_STATUS.IDLE:
    default:
      return translationFn('customer.incidents.ai.status_idle', 'Chưa phân tích');
  }
};
