...
import { Server, Bell, ShieldAlert, AlertTriangle, ArrowRight } from 'lucide-react';
              ...
              <div key={alert._id || i} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.8571rem' }}>
                  <div style={{
                    ...
                    background: severityColor[alert.severity?.toLowerCase()] || 'var(--custom-color-14)',
                  }} />
                  <div>
                    <p style={{ margin: 0, fontSize: '1rem', color: 'var(--slate-900)', fontWeight: '500' }}>
                      ...
                      {alert.device_id?.current_scenario && alert.device_id.current_scenario !== 'NORMAL' && (
                        <span style={{ marginLeft: '8px', padding: '2px 6px', fontSize: '10px', background: 'var(--orange-500)', color: 'white', borderRadius: '4px' }}>
                          Mô phỏng: {getScenarioLabel(alert.device_id.current_scenario)}
                        </span>
                      )}
                    </p>
                    <div style={{ display: 'flex', gap: '0.7143rem', alignItems: 'center' }}>
                      <p style={{ margin: 0, fontSize: '0.8571rem', color: 'var(--slate-500)' }}>
                ...
                <span style={{
                  ...
                  {alert.severity ? t(`customer.severity.${alert.severity.toLowerCase()}`, alert.severity) : t('customer.common.unknown')}
                </span>
              </div>
            ))}
          </div>
